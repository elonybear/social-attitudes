...
export default class MessageWindow extends React.Component {
  ...
  renderMessages() {
    return this.state.toRender.map((message, index) => {
      let messageClass = message.type.toLowerCase();
      return (
        <div className={`msg-wrapper`} key={index}>
          <div className={`msg-${messageClass}`}>
            <div className="msg-author">
              <div className={`msg-author-inner-${messageClass}`}>{message.author}</div>
            </div>
            <div className={`msg-text msg-${messageClass}-text`}>
              {message.text}
            </div>
          </div>
        </div>
  ...
  renderParticipants() {
    let botNames = this.props.users.edges.map(edge => edge.node.first_name + " " + edge.node.last_name);
